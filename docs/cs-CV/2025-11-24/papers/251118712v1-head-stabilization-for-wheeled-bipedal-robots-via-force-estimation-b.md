...
**提出基于力估计的导纳控制以解决轮式双足机器人头部在不平地形中的稳定性问题**

**关键词**: `轮式双足机器人` `头部稳定性` `力估计` `导纳控制` `地形适应性` `仿真实验`

## 📋 核心要点

1. 核心问题：轮式双足机器人在不平地形中头部不稳定，影响传感器精度和载荷安全
2. 方法要点：开发基于模型的接地力估计方法，并应用导纳控制算法增强地形适应性
3. 实验或效果：仿真实验验证了力估计器的实时性能和机器人在不平地形中的鲁棒性
