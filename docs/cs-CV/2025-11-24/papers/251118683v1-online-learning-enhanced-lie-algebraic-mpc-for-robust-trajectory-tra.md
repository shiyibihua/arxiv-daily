...
**提出结合李群MPC与在线学习的控制器，以解决自主水面艇在未知扰动下的轨迹跟踪问题。**

**关键词**: `自主水面艇` `轨迹跟踪` `李群MPC` `在线学习` `扰动补偿` `鲁棒控制`

## 📋 核心要点

1. 核心问题：自主水面艇易受风浪等环境扰动影响，轨迹跟踪在动态海洋条件下困难。
2. 方法要点：结合李群上的凸误差状态MPC与在线学习模块，实时补偿未知扰动。
3. 实验或效果：在仿真和真实实验中，相比现有方法，在各种扰动场景下实现更高跟踪精度。
