...
**提出FID-Net以解决无人机可见光图像中森林病虫害树木的检测与空间分析问题**

**关键词**: `森林病虫害检测` `无人机图像分析` `深度学习模型` `特征增强` `空间分析` `YOLOv8`

## 📋 核心要点

1. 核心问题：传统方法在大规模、细粒度森林病虫害检测中存在局限性，需高效监测以维护生态系统稳定。
2. 方法要点：基于YOLOv8n，引入特征增强模块、自适应多尺度特征融合模块和高效通道注意力机制，提升检测精度。
3. 实验或效果：在32个森林样地数据集上，FID-Net在精度、召回率和mAP指标上优于主流YOLO模型，并支持空间分析。
