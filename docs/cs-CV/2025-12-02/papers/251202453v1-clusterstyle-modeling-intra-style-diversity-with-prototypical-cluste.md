...
**提出ClusterStyle框架，通过原型聚类建模风格内多样性，以提升风格化运动生成效果。**

**关键词**: `风格化运动生成` `原型聚类` `风格内多样性` `运动风格迁移` `Stylistic Modulation Adapter`

## 📋 核心要点

1. 核心问题：现有模型难以捕捉风格内多样性，即同一风格对应多种运动变化。
2. 方法要点：利用原型集建模全局和局部风格多样性，通过Stylistic Modulation Adapter集成风格特征。
3. 实验或效果：在风格化运动生成和运动风格迁移任务中优于现有先进模型。
