...
**提出ViSAudio端到端框架，从无声视频直接生成双耳空间音频以解决现有方法误差累积问题。**

**关键词**: `视频到音频生成` `双耳空间音频` `端到端框架` `条件流匹配` `时空对齐`

## 📋 核心要点

1. 核心问题：现有视频到音频生成方法多输出单声道，双耳音频生成依赖两阶段流程导致误差累积和时空不一致。
2. 方法要点：采用条件流匹配与双分支音频生成架构，结合条件时空模块平衡声道一致性与空间特性。
3. 实验或效果：在BiAudio数据集上验证，ViSAudio在客观指标和主观评估中优于现有方法，生成高质量空间音频。
