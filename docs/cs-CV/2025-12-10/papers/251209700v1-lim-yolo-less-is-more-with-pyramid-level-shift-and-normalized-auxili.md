...
**提出LiM-YOLO，通过金字塔层级偏移与归一化辅助分支解决遥感图像中船舶检测的尺度差异问题。**

**关键词**: `船舶检测` `遥感图像` `金字塔层级偏移` `归一化辅助分支` `目标检测` `尺度差异`

## 📋 核心要点

1. 核心问题：遥感图像中船舶目标尺度差异大且形态各向异性，导致通用检测器在小目标上性能下降。
2. 方法要点：基于统计分析，采用金字塔层级偏移策略调整检测头至P2-P4，并引入GN-CBLinear模块增强训练稳定性。
3. 实验或效果：在多个数据集上验证，LiM-YOLO在检测精度和效率上优于现有先进模型。
