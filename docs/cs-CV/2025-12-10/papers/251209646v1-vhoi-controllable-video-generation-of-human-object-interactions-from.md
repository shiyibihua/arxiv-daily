...
**提出VHOI框架，通过运动稠密化从稀疏轨迹生成可控的人-物交互视频**

**关键词**: `可控视频生成` `人-物交互` `运动稠密化` `视频扩散模型` `稀疏轨迹`

## 📋 核心要点

1. 核心问题：现有可控视频生成方法在稀疏控制（易指定但缺乏实例感知）与稠密信号（信息丰富但获取成本高）间存在权衡。
2. 方法要点：采用两阶段框架，先稠密化稀疏轨迹为人-物交互掩码序列，再基于掩码微调视频扩散模型，引入人-物交互感知运动表示。
3. 实验或效果：在可控人-物交互视频生成中实现先进结果，并能端到端生成包含导航的完整交互场景。
