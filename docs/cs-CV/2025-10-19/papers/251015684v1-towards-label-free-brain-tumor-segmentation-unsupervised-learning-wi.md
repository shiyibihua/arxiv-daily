...
**提出多模态视觉变换器自编码器，用于无监督脑肿瘤分割以解决标注数据稀缺问题。**

**关键词**: `无监督异常检测` `脑肿瘤分割` `多模态MRI` `视觉变换器` `重建误差图` `标签高效工具`

## 📋 核心要点

1. 核心问题：脑肿瘤分割中标注数据有限、昂贵或不一致，限制监督学习可扩展性。
2. 方法要点：使用健康脑MRI训练自编码器，通过重建误差图检测肿瘤，并融合多模态MRI序列。
3. 实验或效果：在BraTS-GoAT数据集上，Dice系数达0.437（全肿瘤），检测率89.4%。
