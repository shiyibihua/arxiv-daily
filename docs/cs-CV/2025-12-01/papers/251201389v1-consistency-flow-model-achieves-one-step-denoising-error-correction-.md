...
**提出ECCFM框架实现一步解码，解决纠错码解码中迭代采样导致的低延迟挑战。**

**关键词**: `纠错码解码` `概率流ODE` `一步推理` `低延迟通信` `神经解码器`

## 📋 核心要点

1. 核心问题：神经解码器在准确性与计算效率间存在权衡，迭代采样限制低延迟应用。
2. 方法要点：基于概率流ODE，通过微分时间正则化学习从噪声信号到原始码字的一步映射。
3. 实验或效果：在多个基准测试中实现更低误码率，推理速度比扩散解码器快30-100倍。
