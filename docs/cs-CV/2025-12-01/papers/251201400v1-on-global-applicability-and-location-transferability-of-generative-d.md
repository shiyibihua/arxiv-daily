...
**评估生成式深度学习模型在全球降水降尺度中的泛化性能与位置可迁移性**

**关键词**: `降水降尺度` `生成式深度学习` `泛化性能` `位置可迁移性` `全球气候数据` `ERA5再分析`

## 📋 核心要点

1. 核心问题：现有降水降尺度模型多为区域特定，泛化到未见地理区域的能力未知。
2. 方法要点：使用全球框架，以ERA5再分析数据为预测因子，IMERG降水估计为目标，进行分层位置数据分割。
3. 实验或效果：系统评估模型在全球15个区域的性能，探索生成式模型的泛化能力。
