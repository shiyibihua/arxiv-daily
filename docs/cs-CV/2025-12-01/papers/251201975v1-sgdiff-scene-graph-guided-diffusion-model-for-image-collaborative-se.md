...
**提出SGDiff模型，利用场景图引导扩散过程，实现基于简单提示的图像协同分割与描述任务。**

**关键词**: `图像协同分割与描述` `场景图引导扩散模型` `多模态对齐` `可控图像理解` `扩散过程`

## 📋 核心要点

1. 核心问题：传统可控图像理解任务需高成本提示或输出有限，难以从简单提示生成多样语义结果。
2. 方法要点：通过提示中心场景图适配器捕捉用户意图，结合场景图引导双模态Transformer预测对齐的掩码-描述对。
3. 实验或效果：在两个数据集上验证，SGDiff在分割与描述任务中表现优异，以最小提示输入获得良好结果。
