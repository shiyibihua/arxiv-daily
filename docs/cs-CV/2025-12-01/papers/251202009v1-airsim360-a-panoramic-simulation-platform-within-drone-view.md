...
**提出AirSim360全景仿真平台，以无人机视角生成大规模全景数据，解决360度理解中数据缺乏问题。**

**关键词**: `全景仿真` `无人机视角` `空间智能` `数据生成` `行人建模` `导航任务`

## 📋 核心要点

1. 核心问题：360度全方位理解领域缺乏大规模多样化数据，限制空间智能发展。
2. 方法要点：采用渲染对齐的数据与标注范式，支持像素级几何、语义和实体级理解；集成交互式行人感知系统建模人类行为；自动化轨迹生成支持导航任务。
3. 实验或效果：收集超过60K全景样本，通过多任务实验验证平台有效性，并公开平台工具与数据集。
