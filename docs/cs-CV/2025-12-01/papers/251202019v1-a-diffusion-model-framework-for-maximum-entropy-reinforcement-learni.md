...
**提出基于扩散模型的框架以改进最大熵强化学习，提升连续控制任务的性能与样本效率。**

**关键词**: `扩散模型` `最大熵强化学习` `策略优化` `连续控制` `样本效率`

## 📋 核心要点

1. 核心问题：将最大熵强化学习重新解释为扩散模型采样问题，以处理复杂策略分布。
2. 方法要点：通过最小化反向KL散度的上界，结合扩散动态推导改进的代理目标。
3. 实验或效果：在标准基准上，DiffSAC、DiffPPO和DiffWPO优于SAC和PPO，实现更高回报和样本效率。
