...
**系统化分析模型上下文协议生态中的安全与安全风险，提出分类与防御路线图**

**关键词**: `模型上下文协议` `安全风险分类` `对抗性威胁` `认知性危害` `多代理环境` `防御路线图`

## 📋 核心要点

1. 核心问题：模型上下文协议（MCP）作为LLM连接外部数据和工具的标准，模糊了幻觉与安全漏洞的边界，引入新威胁。
2. 方法要点：建立风险分类学，区分对抗性安全威胁（如间接提示注入）和认知性安全危害（如分布式工具委托中的对齐失败）。
3. 实验或效果：分析MCP原语（资源、提示、工具）的结构性漏洞，并调查从加密来源到运行时意图验证的先进防御方法。
