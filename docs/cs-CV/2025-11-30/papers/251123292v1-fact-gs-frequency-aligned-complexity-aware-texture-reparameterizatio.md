...
**提出FACT-GS框架，通过频率对齐的纹理重参数化解决高斯溅射中纹理采样效率低的问题。**

**关键词**: `高斯溅射` `纹理重参数化` `自适应采样` `实时渲染` `频率对齐`

## 📋 核心要点

1. 核心问题：传统高斯溅射使用均匀纹理采样，导致高频区域欠采样和平滑区域浪费容量，影响细节表现。
2. 方法要点：基于自适应采样理论，引入可学习的频率感知分配策略，通过变形场的雅可比矩阵调制局部采样密度。
3. 实验或效果：在相同参数预算下，恢复更锐利的高频细节，同时保持实时渲染性能。
