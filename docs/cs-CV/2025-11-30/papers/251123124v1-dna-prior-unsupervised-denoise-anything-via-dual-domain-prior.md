...
**提出DNA-Prior，一种无监督通用去噪框架，通过双域先验解决医学图像去噪问题。**

**关键词**: `无监督去噪` `医学图像处理` `双域先验` `优化框架` `噪声抑制`

## 📋 核心要点

1. 核心问题：医学图像去噪依赖标注数据或监督学习，在异构模态和有限真值数据下受限。
2. 方法要点：结合隐式架构先验和显式谱-空间先验，形成双域优化问题，无需外部训练数据。
3. 实验或效果：在多模态实验中，DNA-Prior在不同噪声条件下实现一致的噪声抑制和结构保持。
