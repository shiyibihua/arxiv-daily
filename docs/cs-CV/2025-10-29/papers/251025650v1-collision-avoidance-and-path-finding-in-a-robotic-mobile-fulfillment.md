...
**arXiv**: [2510.25650v1](https://arxiv.org/abs/2510.25650) | [PDF](https://arxiv.org/pdf/2510.25650.pdf)
...
**提出多目标元启发式算法以优化机器人移动履行系统中的避碰与路径规划**

**关键词**: `多智能体路径规划` `碰撞避免` `任务分配` `多目标优化` `元启发式算法` `自动导引车`

## 📋 核心要点

1. 核心问题：解决AGV之间的碰撞和任务分配，同时考虑能耗与旅行时间。
2. 方法要点：引入新避碰策略，并应用NSGA和ALNS多目标算法。
3. 实验或效果：比较评估显示，方法在避碰和任务分配上优于现有方法。

## 📄 摘要（原文）

> Multi-Agent Path Finding (MAPF) has gained significant attention, with most
