...
**提出仿真测试框架Surrealist，应用于ANYmal四足机器人工业检测，以自动生成障碍规避场景。**

**关键词**: `机器人导航测试` `仿真测试生成` `障碍规避` `工业机器人` `搜索算法` `ANYmal机器人`

## 📋 核心要点

1. 核心问题：动态环境中机器人导航鲁棒性不足，传统测试方法难以覆盖全操作需求。
2. 方法要点：基于搜索算法自动生成挑战性障碍规避测试场景，提升故障发现能力。
3. 实验或效果：在工业评估中测试五款专有算法，通过调查确认增强开发流程和验证。
