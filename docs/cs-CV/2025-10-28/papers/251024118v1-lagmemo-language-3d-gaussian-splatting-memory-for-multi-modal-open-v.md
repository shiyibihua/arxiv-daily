...
**提出LagMemo系统，利用语言3D高斯泼溅记忆解决多模态开放词汇多目标视觉导航问题**

**关键词**: `视觉导航` `多模态学习` `开放词汇` `3D高斯泼溅` `多目标导航` `语言记忆`

## 📋 核心要点

1. 核心问题：传统视觉导航方法局限于单目标、单模态和封闭集目标设置，无法满足多模态开放词汇多目标需求。
2. 方法要点：构建统一3D语言记忆，通过查询预测候选目标位置，并集成局部感知验证机制动态匹配目标。
3. 实验或效果：在GOAT-Core基准上，LagMemo在开放词汇目标定位和多目标导航中优于现有方法。
